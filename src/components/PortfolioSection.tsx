  ...
  const filteredProjects = useMemo(() => {
    return projects.filter((project) => {
      const matchesCategory =
        ...
        (project.secondaryCategory && project.secondaryCategory.toLowerCase() === selectedCategory.toLowerCase());
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-slate-600 text-sm sm:text-base mt-2 max-w-2xl">
              Browse our track record of completed waterproofing membranes, high-strength industrial epoxy screeds, and protective architectural coatings.
            </p>
          </div>
          ...
          {isAdminLoggedIn && (
            <button
              onClick={onOpenAdmin}
              className="inline-flex items-center gap-2 px-4 py-2.5 text-xs font-semibold text-slate-800 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-xl transition-all cursor-pointer"
            >
        ...
        {!loading && filteredProjects.length === 0 && (
          <div className="text-center py-16 bg-slate-50 rounded-2xl border border-dashed border-slate-300">
            <p className="text-slate-600 text-sm font-medium">No projects found matching the selected filter or query.</p>
            <button
              onClick={() => {
            ...
            >
              ...
              <div>
                <div className="relative h-52 overflow-hidden bg-slate-100">
                  ...
                  <div className="absolute top-3 right-3 flex items-center gap-1.5">
                    ...
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded shadow-xs ${
                  ...
                  {/* Client name overlay on bottom left */}
              ...
              <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/70 flex items-center justify-between text-xs text-slate-500">
                <div className="flex items-center gap-1">
                  ...
                  <span>{project.areaSqFt ? `${project.areaSqFt.toLocaleString()} sq.ft.` : 'Custom Scope'}</span>
                </div>
